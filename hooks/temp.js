    ...
    useEffect(() => {
        if (
            ...
            const fetchPools = async () => {
                ...
                let live = poolsData.filter((pool) => Number(pool.allocPoint) > 0)
                // 更新状态
                let finished = poolsData.filter((pool) => Number(pool.allocPoint) === 0)
                setFinishedPools(finished)

                // 准备 multicall 调用 lpToken
                const poolCalls2 = Array.from({ length: live.length }, (e, i) => ({
                    address: masterChefV2Address,
                ...
                const poolsDataAddLpToken = live.map((e, i) => {
                    return {
                        ...e,
                ...
                poolsDataAddTotalStaked = poolsDataAddTotalStaked.map((e, i) => ({
                    ...
                    totalStaked: results3[i].result ? formatEther(results3[i].result) : '0'
                }))
                ...
                for (const e of updatedArrayA) {
                    if (e.token0 && e.token1) {
                        e.cakeStakedInUsdt =
                            Number(e.totalStaked) * Number(formatUnits(cakePrice, 8))
                    }
                }

                setLivePools(updatedArrayA)
                setLoading(false)
            }
            fetchPools()
        }
